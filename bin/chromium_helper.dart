...
class ChromiumHelper {
  ...
  ///
  static Future<RevisionInfo> justDownloadChrome({int? revision, String? cachePath}) async {
    ...
    return RevisionInfo(folderPath: revisionDirectory.path, executablePath: zipFile.path, revision: revision);
  }

  /// use in case when user just want to extract their downloaded chromium file
  ///
  static Future<RevisionInfo> justExtractChrome({int? revision, String? cachePath}) async {
    revision ??= ChromiumInfoConfig.lastRevision;
    cachePath ??= '.local-chromium';
  ...
  ///
  static Future<RevisionInfo> downloadChrome({int? revision, String? cachePath}) async {
    revision ??= ChromiumInfoConfig.lastRevision;
    cachePath ??= '.local-chromium';
...
//https://github.com/maxogden/extract-zip/blob/master/index.js
  ...
  static const _baseUrl = 'https://storage.googleapis.com/chromium-browser-snapshots';
