...
import 'package:webcontent_converter_plus/revision_info.dart';
...
class ChromeDesktopDirectoryHelper {
  ...
  static String? assetChromeZipPath() {
    ...
    return appsDirPath.isEmpty
        ...
        : p.joinAll(['assets', appsDirPath, '.local-chromium', '${revision}_$filename']);
  }
  ...
  static FutureOr<String> saveChromeFromAssetToApp({
    ...
    final tagetDirectory = io.Directory(p.joinAll([targetPath, zipFileName().replaceAll(".zip", "")]));
    ...
    final zipPath = io.Directory(p.joinAll([(await path.getApplicationSupportDirectory()).path, zipFileName()])).path;
    ...
    if (!executableFile.existsSync()) {
      ...
      if (!zipFile.existsSync()) {
        ...
        Uint8List wzzip = value.buffer.asUint8List(value.offsetInBytes, value.lengthInBytes);
...
//https://github.com/maxogden/extract-zip/blob/master/index.js
  ...
  static FutureOr<String> applicationSupportPath() async {
    ...
    return appsDirPath.isEmpty
        ? io.Directory(p.joinAll([supportDir.path, '.local-chromium', '$revision'])).absolute.path
        : io.Directory(p.joinAll([supportDir.path, appsDirPath, '.local-chromium', '$revision'])).absolute.path;
  }
  ...
  static FutureOr<String> getChromeExecutablePath() {
    if (io.Platform.isWindows) {
      ...
      return p.join('chrome-mac', 'Chromium.app', 'Contents', 'MacOS', 'Chromium');
    } else {
