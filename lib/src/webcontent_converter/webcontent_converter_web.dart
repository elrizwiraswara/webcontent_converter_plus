...
class WebcontentConverter {
  ...
  static Future<Uint8List> contentToImage({
    ...
    List<int> result = [];
    html.CanvasElement? canvas = await promiseToFuture(html2canvas(div, jsify(opt)));
    if (canvas != null) {
  ...
  /// [embedWebView]
  static Widget embedWebView({String? url, String? content, double? width, double? height}) => Builder(builder: (_) {
  ...
  static Future<bool> printPreview({
    ...
    try {
      ...
      js.JsObject printWindow = js.context.callMethod('open', [url ?? '', "mozillaWindow", windowFeatures]);
      js.JsObject? document = printWindow.hasProperty('document') ? printWindow['document'] : null;
      // ref: https://developer.mozilla.org/en-US/docs/Web/API/Document

      js.JsObject? window = printWindow.hasProperty('window') ? printWindow['window'] : null;
