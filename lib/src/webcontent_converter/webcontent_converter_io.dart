...
class WebcontentConverter {
  static const MethodChannel _channel = const MethodChannel('webcontent_converter');
  ...
  static Future<void> ensureInitialized({
    ...
    if (windowBrower == null || windowBrower?.isConnected != true) {
      await WebcontentConverter.initWebcontentConverter(executablePath: executablePath, content: content);
    }
  }
  ...
  static Future<void> initWebcontentConverter({
    ...
    if (io.Platform.isMacOS || io.Platform.isLinux || io.Platform.isWindows) {
      ...
      preloadBytes = await contentToImage(content: content);
    }
      ...
      if (io.Platform.isMacOS || io.Platform.isLinux || io.Platform.isWindows) {
        if (WebViewHelper.isChromeAvailable) {
          ...
          try {
            ...
            if (windowBrower == null || windowBrower?.isConnected != true) {
              windowBrower = await pp.puppeteer
                  .launch(headless: true, executablePath: executablePath ?? WebViewHelper.executablePath());
            }
            ...
            windowBrowserPage.setViewport(pp.DeviceViewport(deviceScaleFactor: scale));
            await windowBrowserPage.emulateMediaType(pp.MediaType.print);
            var offsetHeight = await windowBrowserPage.evaluate('document.body.offsetHeight');
            var offsetWidth = await windowBrowserPage.evaluate('document.body.offsetWidth');
            results = await windowBrowserPage.screenshot(
              format: pp.ScreenshotFormat.png,
              clip: pp.Rectangle.fromPoints(pp.Point(0, 0), pp.Point(offsetWidth, offsetHeight)),
              fullPage: false,
              omitBackground: true,
            );
          } catch (e) {
  ...
  static Future<bool> printPreview({
    ...
    try {
      ...
      if ((io.Platform.isMacOS || io.Platform.isLinux || io.Platform.isWindows) && WebViewHelper.isChromeAvailable) {
        ...
        if (duration != null) await Future.delayed(Duration(milliseconds: duration.toInt()));
